...
describe('Route', () => {
  ...
  const foo = Route.literal('foo').concat(Route.separator, Route.integer('fooId').prefix('foo-'))
  const bar = Route.literal('bar').concat(Route.separator, Route.integer('barId').prefix('bar-'))
  ...
  const concatWithPrefix = Route.parse('/')
    .concat(Route.literal('foo'))
    .concat(Route.literal('bar'))
  ...
  it('generates schemas', async () => {
    ...
    const decodeSeparator = Schema.decode(Route.separator.schema)
    ...
    const test = Effect.gen(function* (_) {
      ...
      deepEqual(yield* _(decodeSeparator({})), {})
  ...
  it('interpolates paths', () => {
    ...
    deepEqual(Route.separator.interpolate({}), '/')
    deepEqual(
      Route.literal('/foo').concat(Route.param('fooId')).interpolate({ fooId: '123' }),
      '/foo/123',
    )
  ...
  it('matches paths', () => {
    ...
    // Separator matches any path that starts with a `/`
    deepEqual(Route.separator.match('/'), Option.some({}))
    deepEqual(Route.separator.match('/foo'), Option.some({}))
    deepEqual(Route.separator.match('/foo/bar'), Option.some({}))
  ...
  it('can be decoded', async () => {
    const test = Effect.gen(function* (_) {
      ...
      deepEqual(yield* _(Route.decode(Route.separator, '/')), {})
  ...
  it('can be encoded', () => {
    const test = Effect.gen(function* (_) {
      ...
      deepEqual(yield* _(Route.encode(Route.separator, {})), '/')
  ...
  it('transforms', async () => {
    const foobarTransformed = foobar.pipe(
      Route.transform(
        Schema.Struct({
          foo: Schema.Number.pipe(Schema.int()),
          bar: Schema.Number.pipe(Schema.int()),
        }),
    ...
    const test = Effect.gen(function* (_) {
      deepEqual(yield* _(foobarTransformed, Route.decode('/foo/foo-1/bar/bar-2')), {
        foo: 1,
        bar: 2,
      })
      deepEqual(
        yield* _(foobarTransformed, Route.encode({ foo: 1, bar: 2 })),
        '/foo/foo-1/bar/bar-2',
      )
    })
  ...
  it('separates path and query schemas', () => {
    const route = Route.literal('/foo').concat(
      Route.param('fooId'),
  ...
  it('allows concatenation of routes with query params', () => {
    const foobar = Route.literal('foo').concat(
      Route.queryParams({ bar: Route.integer('bar').optional() }),
    )
    const bazQuux = Route.literal('baz').concat(
      Route.queryParams({ quux: Route.integer('quux').optional() }),
    )
  ...
  it('can be ordered by complexity', () => {
    ...
    const expected = [feed, comment, favorites, comments, article, articles]

    deepEqual(
      Route.sortRoutes(routes).map((r) => r.path),
