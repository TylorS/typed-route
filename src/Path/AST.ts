...
export type PathToSegments<T extends string> = `` extends T
  ? []
  : T extends `${infer P}${QueryParamsSyntax<infer Q>}`
    ? [...PathToSegments<P>, QueryParamsSyntax<Q>]
    : T extends `${infer P}/${infer R}`
      ? `` extends P
        ? PathToSegments<R>
        : [...PathToSegments<P>, ...PathToSegments<R>]
      : [T]

export type QueryParamsToSegments<T extends string> = `` extends T
  ? []
  : T extends QueryParamsSyntax<infer T>
    ? QueryParamsToSegments<T>
    : T extends `${infer P}&${infer R}`
      ? [...QueryParamsToSegments<P>, ...QueryParamsToSegments<R>]
      : [T]
...
export type NamedParamNode<in out T extends string> = {
  readonly _tag: 'NamedParam'
  readonly name: TextNode<T>
}

export type NamedParamNodeWithRegex<in out T extends string, in out Regex extends string> = {
  readonly _tag: 'NamedParamWithRegex'
  readonly name: TextNode<T>
  readonly options: TextNode<Regex>
}
...
export type PrefixNode<in out T extends string, in out P extends SegmentAST | null> = {
  ...
  readonly prefix: TextNode<T>
  readonly param: P
}

export type MakePrefixNode<T extends string, P extends SegmentAST | null> = `` extends T
  ? P
  : PrefixNode<T, P>

export type SuffixNode<in out P extends SegmentAST | null, in out T extends string> = {
  readonly _tag: 'Suffix'
  readonly param: P
  readonly suffix: TextNode<T>
}

export type MakeSuffixNode<P extends SegmentAST | null, T extends string> = `` extends T
  ? P
  : SuffixNode<P, T>

export type QueryParamsNode<
  in out Params extends ReadonlyArray<TextNode<any> | QueryParamNode<any, any>>,
> = {
...
type ParseSegments_<T extends ReadonlyArray<string>, Count extends number> = T extends readonly [
  infer Head,
...
type ParseExactSegment<T, Count extends number> = T extends QueryParamsSyntax<infer P>
  ...
  : T extends ModifierSyntax<infer M, '?'>
    ? ModifierNode<'?', ParseSegment<M, Count>>
    : T extends ModifierSyntax<infer M, '*'>
      ? ModifierNode<'*', ParseSegment<M, Count>>
      : T extends ModifierSyntax<infer M, '+'>
        ? ModifierNode<'+', ParseSegment<M, Count>>
        : T extends Prefix<infer P, Unnamed>
          ? PrefixNode<P, UnnamedParamNode<Count>>
          : T extends Prefix<infer P, NamedParamWithRegexSyntax<infer S, infer Regex>>
            ? PrefixNode<P, NamedParamNodeWithRegex<S, Regex>>
            : T extends Prefix<infer P, Param<infer S>>
              ? PrefixNode<P, NamedParamNode<S>>
              : T extends Unnamed
                ? UnnamedParamNode<Count>
                : T extends NamedParamWithRegexSyntax<infer P, infer Regex>
                  ? NamedParamNodeWithRegex<P, Regex>
                  : T extends Param<infer P>
                    ? // Special cases to ensure that suffixes in params are parsed correctly
                      P extends `${infer P2}+${infer Tail}`
                      ? MakeSuffixNode<ModifierNode<'+', NamedParamNode<P2>>, Tail>
                      : P extends `${infer P2}*${infer Tail}`
                        ? MakeSuffixNode<ModifierNode<'*', NamedParamNode<P2>>, Tail>
                        : P extends `${infer P2}?${infer Tail}`
                          ? MakeSuffixNode<ModifierNode<'?', NamedParamNode<P2>>, Tail>
...
type ParseSegmentWithImplicitPrefixAndSuffix<
  T,
    ...
    : T extends `${infer Head}${ModifierSyntax<infer M, '+'>}${infer Tail}`
      ...
      : T extends `${infer Head}${Prefix<infer P, Unnamed>}${infer Tail}`
        ...
        : T extends `${infer Head}${Prefix<infer P, NamedParamWithRegexSyntax<infer S, infer Regex>>}${infer Tail}`
          ? MakePrefixNode<
              Head,
              MakeSuffixNode<MakePrefixNode<P, NamedParamNodeWithRegex<S, Regex>>, Tail>
            >
          : T extends `${infer Head}${Prefix<infer P, Param<infer S>>}${infer Tail}`
            ...
            : T extends `${infer Head}${Unnamed}${infer Tail}`
              ...
              : T extends `${infer Head}${NamedParamWithRegexSyntax<infer P, infer Regex>}${infer Tail}`
                ...
                : T extends `${infer Head}${Param<infer P>}`
                  ? MakePrefixNode<Head, NamedParamNode<P>>
...
export type ParseQueryParamSegments<
  T extends ReadonlyArray<string>,
  Count extends number,
> = T extends readonly [infer Head, ...infer Tail extends ReadonlyArray<string>]
  ...
  : T extends readonly [infer Head]
    ? [ParseQueryParamSegment<Head, Count>]
    : []
...
type HasUnnamedParam<T> = T extends UnnamedParamNode<infer _>
  ? true
  : T extends ModifierNode<any, infer S>
    ? HasUnnamedParam<S>
    : T extends PrefixNode<any, infer S>
      ? HasUnnamedParam<S>
      : T extends SuffixNode<infer S, any>
        ? HasUnnamedParam<S>
        : T extends QueryParamsNode<infer Q>
          ? HasUnnamedParamInQueryParams<Q>
          : false

type HasUnnamedParamInQueryParams<T extends ReadonlyArray<SegmentAST | QueryParamNode<any, any>>> =
  T extends readonly [
    infer Head,
    ...infer Tail extends ReadonlyArray<SegmentAST | QueryParamNode<any, any>>,
  ]
    ? HasUnnamedParam<Head> extends true
      ? true
      : HasUnnamedParamInQueryParams<Tail>
    : false
